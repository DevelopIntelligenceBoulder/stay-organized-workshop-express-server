...
app.post("/api/todos", function (request, response) {
    ...
    if (!userid || !category || !description || !deadline || !priority) {
        console.warn(
            "LOG: **MISSING DATA**: one or more todo properties missing"
        );

        response.status(400).json({
            error: "Missing data, can't process: one or more Todo properties missing.",
        });
    ...
    const nextIdJson = fs.readFileSync(
